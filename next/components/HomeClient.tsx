...
import { createEditorialStatusDataset, editorialStatusFromSearchParam, filterEditorialStatusDataset, PUBLIC_EDITORIAL_STATUSES, type PublicEditorialStatus } from "@/lib/domain/coverage-explorer";
import { getPublicEntry_Client } from "@/lib/data/client-search";
import type { SonataEntryCard } from "@/lib/data/sonata-demo";
...
export function HomeClient({
  ...
  initialEntries: SonataEntryCard[];
  ...
  const [editorialStatus, setEditorialStatus] = useState<PublicEditorialStatus | "all">(() =>
    editorialStatusFromSearchParam(typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("status")),
  );
  const [featuredIndex, setFeaturedIndex] = useState(0);

  const searchResults = useMemo(() => (query.trim().length > 0 ? getPublicEntry_Client(initialEntries, query) : []), [query, initialEntries]);

  const sourceEntries = query.trim().length > 0 ? searchResults : initialEntries;
  const collectionFilters = ["All pathways", ...(coverage.regions ?? []).map(region => region.label)];
  const visibleEntries = useMemo(() => {
    if (!query.trim().length && activeFilter === "All pathways") return sourceEntries;
    return sourceEntries.filter(entry => entry.region === activeFilter || entry.region.includes(activeFilter));
  }, [activeFilter, sourceEntries, query]);

  const visibleCoverageTargets = useMemo(
    () =>
      (coverage.targets ?? [])
        .filter(target => target.dimension === coverageDimension)
        .filter(target =>
          coverageStatus === "all" ? true : coverageStatus === "published" ? target.publishedCount > 0 : target.publishedCount < target.targetCount,
        ),
    [coverage.targets, coverageDimension, coverageStatus],
  );

  const editorialStatusDataset = useMemo(() => createEditorialStatusDataset(coverage.editorialStatusCounts), [coverage.editorialStatusCounts]);
  const visibleStatusDataset = filterEditorialStatusDataset(editorialStatusDataset, editorialStatus);
  const selectedStatusCount =
    editorialStatus === "all" ? (coverage.draftConcepts ?? 0) + (coverage.publishedConcepts ?? 0) : coverage.editorialStatusCounts?.[editorialStatus] ?? 0;

  const featuredEntries = initialEntries;
        ...
        <main className="hero">
          ...
          <div className="hero__copy">
            ...
            <div className="coverage-signal" aria-label="Knowledge coverage target">
              <span>Part 2</span>
              <strong>{(coverage.primaryTarget ?? 15350).toLocaleString()} source-aware coverage target</strong>
              <small>{coverage.publishedConcepts ?? 0} published · {coverage.draftConcepts ?? 0} editorial records</small>
            </div>
          </div>
      ...
      <section className="discovery-section" id="discover">
        ...
        <div className="discovery-section__content">
          ...
          <div className="search-deck">
            <label className="search-field" htmlFor="sonata-search">
              ...
              <input
                ...
                onKeyDown={event => {
                  if (event.key === "Enter") router.push(`/search?q=${encodeURIComponent(query)}`);
                }}
          ...
          <div className="discovery-modes" aria-label="Sonata discovery pathways">
            <Link href="/search?q=Region">
              <Compass size={18} />
              <span>
                <strong>Explore by region</strong>
                <small>Place, lineage, and local context</small>
              </span>
              <ArrowRight size={15} />
            </Link>
            <Link href="/search?q=Tradition">
              <Layers3 size={18} />
              <span>
                <strong>Follow a tradition</strong>
                <small>Systems, repertoires, and practices</small>
              </span>
              <ArrowRight size={15} />
            </Link>
            <Link href="/compare">
              ...
              <span>
                <strong>Compare concepts</strong>
                <small>Difference before equivalence</small>
              </span>
              <ArrowRight size={15} />
            </Link>
            <Link href="/learn">
              ...
              <span>
                <strong>Build a learning path</strong>
                <small>Published knowledge, progressively read</small>
              </span>
              <ArrowRight size={15} />
            </Link>
            <Link href="/assistant">
              ...
              <span>
                ...
                <small>Source-grounded synthesis</small>
              </span>
              <ArrowRight size={15} />
            </Link>
          </div>
          ...
          {featuredEntry ? (
            <section className="explore-concept" aria-label="Explore a concept">
              <div>
                <p className="eyebrow">Explore a concept</p>
                <p>Let one researched record open the next pathway.</p>
                <button type="button" onClick={() => setFeaturedIndex(index => index + 1)}>
                  Another record <Sparkles size={14} />
                </button>
              </div>
              <Link href={`/entries/${featuredEntry.slug}`}>
                <span>{String(featuredIndex + 1).padStart(2, "0")}</span>
                <div>
                  <p className="eyebrow">
                    {featuredEntry.tradition} · {featuredEntry.region}
                  </p>
                  <h3>{featuredEntry.name}</h3>
                  <p>{featuredEntry.shortDefinition}</p>
                </div>
                <ArrowRight size={20} />
              </Link>
            </section>
          ...
          <section className="coverage-explorer" aria-labelledby="coverage-explorer-heading">
            <div className="coverage-explorer__heading">
              <div>
                <p className="eyebrow">Coverage explorer</p>
                <h3 id="coverage-explorer-heading">Read the editorial map by more than region.</h3>
              </div>
              <p>These filters explore coverage targets and published counts. They do not inflate the public catalogue with unreviewed candidates.</p>
            </div>
            <div className="coverage-explorer__filters" aria-label="Filter coverage targets">
              <div>
                <span>Lens</span>
                {(["region", "tradition", "domain", "era"] as const).map(dimension => (
                  <button
                    type="button"
                    key={dimension}
                    className={`filter-button ${coverageDimension === dimension ? "is-active" : ""}`}
                    onClick={() => setCoverageDimension(dimension)}
                  >
                    {dimension}
                  </button>
                ))}
              ...
              <div>
                ...
                {(["all", "published", "planned"] as const).map(status => (
                  <button
                    type="button"
                    key={status}
                    className={`filter-button ${coverageStatus === status ? "is-active" : ""}`}
                    onClick={() => setCoverageStatus(status)}
                  >
                    {status === "planned" ? "planned gap" : status}
                  </button>
                ))}
              </div>
            </div>
            <div className="editorial-status-lens">
              <div>
                <span>Editorial status lens</span>
                <div>
                  {(["all", ...PUBLIC_EDITORIAL_STATUSES] as const).map(status => (
                    <button
                      type="button"
                      key={status}
                      className={`filter-button ${editorialStatus === status ? "is-active" : ""}`}
                      onClick={() => {
                        setEditorialStatus(status);
              ...
              <p>
                <strong>{selectedStatusCount}</strong> aggregate {editorialStatus === "all" ? "tracked records" : `${editorialStatus.replace(/_/g, " ")} records`}. Only published records are available as public entries; other statuses remain aggregate-only to protect the editorial workflow.
              </p>
            </div>
            <div className="editorial-status-dataset" aria-live="polite">
              {visibleStatusDataset.map(row => (
                <article key={row.status}>
                  <span>{row.status.replace(/_/g, " ")}</span>
                  <strong>{row.count}</strong>
                  <small>{row.status === "published" ? "public entries" : "aggregate only"}</small>
                </article>
              ))}
            </div>
            <div className="coverage-explorer__cards">
              {visibleCoverageTargets.length ? (
                visibleCoverageTargets.map(target => (
                  <article key={`${target.dimension}-${target.slug}`}>
                    <p>{target.label}</p>
                    <strong>{target.targetCount.toLocaleString()}</strong>
                    <span>
                      {target.publishedCount} published · {Math.max(0, target.targetCount - target.publishedCount).toLocaleString()} planned gap
                    </span>
                  </article>
                ))
              ) : (
                <p className="coverage-explorer__empty">
                  No targets have reached the selected public-record state yet. This is an honest coverage signal, not an empty-data error.
                </p>
              )}
            </div>
          </section>
          ...
          <div className="discover-grid">
            <aside className="taxonomy-spine" id="atlas">
              ...
              <div className="taxonomy-spine__list">
                {(coverage.regions ?? []).slice(0, 7).map((node, index) => (
                  <button type="button" key={node.slug} className="taxonomy-node" onClick={() => setActiveFilter(node.label)}>
            ...
            <div className="entry-index">
              ...
              <div className="coverage-ledger">
                ...
                <p>
                  <strong>Coverage is planned, not claimed.</strong> Sonata’s {(coverage.primaryTarget ?? 15350).toLocaleString()}-concept target is staged in reviewed batches of 100–500. Low-confidence or unresolved candidates remain outside public discovery.
                </p>
              </div>
              ...
              <div className="entry-grid">
                {visibleEntries.map((entry, index) => (
                  <EntryCard key={entry.publicId} entry={entry} index={index} />
                ))}
              </div>
            </div>
          </div>
      ...
      <section className="method-section" id="method">
        ...
        <div className="method-section__content">
          <div className="method-section__image-wrap">
            <div className="method-section__image method-section__image--placeholder" aria-hidden="true" />
